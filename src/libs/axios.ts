  ...
  async (error: AxiosError) => {
    ...
    if (status === 401 || status === 400) {
      ...
      const refreshTokenFn = async () => {
        try {
          if (userLocal.social !== null && userLocal.social !== undefined) {
            const { data } = await axiosInstance.post('/user/refresh', {
              headers: { 'Content-Type': 'application/json' },
              withCredentials: true,
            });
            // 새로운 토큰 저장
            const { accessToken: newAccessToken, expiresAt: newExpiresAt } = data;
            userLocal.accessToken = newAccessToken;
            userLocal.expiresAt = newExpiresAt;
            localStorage.setItem('auth-store', JSON.stringify(userLocal));
          }
        } catch (error) {
      ...
      try {
        ...
        if (originalRequest) {
          const newAccessToken = userLocal.accessToken; // 업데이트된 토큰 사용
          if (newAccessToken) {
            originalRequest.headers.Authorization = `Bearer ${newAccessToken}`;
            originalRequest.headers.social = userLocal.social;
